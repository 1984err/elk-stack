...
03:46:58.000   system.auth    Connection closed by authenticating user RedAdmin 10.0.0.4 port 57776 [preauth]

2. Generate a high amount of CPU usage on the pen-testing machines and verify that Kibana picks up this data.

RedAdmin@10.0.0.5: Permission denied (publickey).
...
You can only be in one docker at a time when using ssh so I can't run stress on both at the same time.  
...
3. Run `ls` to view the file you downloaded from your web VM to your jump box. 
	...
	```bash
	RedAdmin@JumpBoxProvisioner:~$ ls
	index.html
	```
...
5. Open the Metrics page for the web machine you attacked and answer the following questions:
	...
	- Which of the VM metrics were affected the most from this traffic? CPU Usage and Load
